class Comparison:
    ...
    # MAIN
    def compare_class_or_individual():   
        instance_type_consistency_check(item1, item2) #for class and individual
        ...
        class_type_consistency_check(item1, item2)

        elif item1 in individuals:
        ...
        place_equality_consistent = place_equality_consistency_check(item1, item2) #needed to run subplace_consistency_check
        subplace_consistency_check(item1, item2, place_equality_consistent)
        temporal_granularity_consistency_check (item1, item2) #run this before subinterval since subinterval needs them to be the same temporal unit, but actually, does it matter? No, but it does in the way that I implemented it using the datetime library
        subinterval_consistency_check(item1, item2)
        #property_consistency_check(item1, item2) #need to debug
        interval_equality_consistency_check(item1, item2)
        interval_non_overlap_consistency_check(item1, item2)

    
    def class_type_consistency_check(item1, item2): # item: KG of SPO or Indicator
        ...
        if item2 in classes:
            ...
            if ((type(item1) == type(item2)) and (item1.label == item2.label) and (set(prop1) == set(prop2))): 
                ...
                print("Class type consistent because classes are equal.")
                ...
                return class_type_consistent
            #subsumption
            for x in list(item1.subclasses()):
                if x in list(item2.subclasses()) or x in item2.INDIRECT_equivalent_to or x == item2:
                    ...
                    return class_type_consistent
            for y in list(item2.subclasses()):
    ...
    def instance_type_consistency_check(item1, item2):
        '''
        ...
        '''
        if (item2 in classes and item1 in individuals):
            #item 2 = n, item1 = m
            #parent = item1.is_instance_of[0] #parent class of item1
            #if parent in str(item2.subclasses()))
            ...
            if item1 not in item2.instances(): 
                ...
                if type(item1) == item2: #item1 has type item2
                    for c in item1.is_instance_of:
                        ...
                        if class_type_consistency_check(c, item2) == False:
        ...
        elif (item1 in classes and item2 in individuals):
            #item1 = n, item2 = m
            #parent = item2.is_instance_of[0] #parent class of item2
            #if parent in str(item1.subclasses()))
            ...
            if item2 not in item1.instances(): 
                ...
                if type(item2) == item1: #item2 has type item1
                    for c in item2.is_instance_of:
                        #The instance mij is type inconsistent with nik if the class civ is inconsistent with nik, i.e., Type InConsistency(civ,nik) is true. 
                        if class_type_consistency_check(c, item1) == False:
    ...
    def get_date(beginning1, beginning2, end1, end2): 
    ...
    #might delete
    def get_date_when_none_values(beginning1, beginning2, end1, end2): 
    ...
    def subinterval_consistency_check(item1, item2): 
        '''
        ...
        #no, im using potentially inconsistent when there isn't enough info. so these would all be inconsistent.
                if item2 in individuals: #cannot run this check unless both are instances
        '''
    ...
    def temporal_granularity_consistency_check(item1, item2):
        if item2 in individuals: #cannot run this check unless both are instances
            indicatorSumOf1 = item1.sumOf
            indicatorSumOf2 = item2.sumOf
    ...
    def property_consistency_check(item1, item2):
        '''
        ...
        '''
        if (item1 in classes and item2 in item1.ancestors()) or (item2 in item1.is_a):
        ...
        if (item2 in classes and item1 in item2.ancestors()) or (item1 in item2.is_a):
        #instance_property_consistent = False
        ...
        for r in restrictions:
            ...
            print("property: " , r.property , " value: " , r.value)
                ...
                print(          print(item1 , " is property inconsistent with " , item2 , " because it does not satisfy the value restriction of property " , prop , " defined in " , item2 +".")
    ...
    def interval_equality_consistency_check(item1, item2):
        if item2 in individuals:
        indicatorSumOf1 = item1.sumOf
        timeInterval1 = indicatorSumOf1[0].forTimeInterval
        beginning1 = timeInterval1[0].hasBeginning
        end1 = timeInterval1[0].hasEnd
    ...
    def interval_non_overlap_consistency_check(item1, item2):
        if item2 in individuals:
        indicatorSumOf1 = item1.sumOf
        timeInterval1 = indicatorSumOf1[0].forTimeInterval
        beginning1 = timeInterval1[0].hasBeginning
    ...
    def place_equality_consistency_check(item1, item2):
        '''
        ...
        if item2 in individuals: #item1 and item2 both need to be individuals
            parent = item1.is_instance_of[0] #parent class of item1
    ...
    def subplace_consistency_check(item1, item2, place_equality_consistent):
        '''
        ...
        if item2 in individuals and place_equality_consistent == False:
            #item1
